...
igvFuenteLuz::igvFuenteLuz(const unsigned int _idLuz,
	...
	angulo_foco = 60; // de esta manera la luz se convierte en puntual	
	exponente_foco = 0;
...
void igvFuenteLuz::setDireccionFoco(igvPunto3D pos)
{
	direccion_foco = pos;
}
...
void igvFuenteLuz::aplicar(void) {
	...
	if (encendida) {
		...
		glLightf(idLuz, GL_SPOT_CUTOFF, this->angulo_foco);
		glLightf(idLuz, GL_SPOT_EXPONENT, this->exponente_foco);
	}
